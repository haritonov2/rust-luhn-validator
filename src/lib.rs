pub fn is_valid(code: &str) -> bool {
    ...
    if clean_code.chars().any(|c| !c.is_numeric()) {
        return false
    }

    let reversed_digits: Vec<u32> = clean_code.chars().rev().map(|c| c.to_digit(10).unwrap()).collect();
    let doubled_digits = double_second_digits(reversed_digits);
...
fn double_second_digits(digits: Vec<u32>) -> Vec<u32> {
    ...
    for (index, value) in digits.iter().enumerate() {
        ...
        let doubled_digit = value * 2;

        if doubled_digit > 9 {
            result.push(doubled_digit - 9);
            continue;
        }

        result.push(doubled_digit);
    }
